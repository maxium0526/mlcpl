...
class PartialAsymmetricLoss(nn.Module):

    def __init__(self, clip=0, gamma_pos=0, gamma_neg=1, gamma_unann=2, alpha_pos=1, alpha_neg=1, alpha_unann=1):
        super(PartialAsymmetricLoss, self).__init__()
        self.clip = clip
        self.gamma_pos = gamma_pos
        self.gamma_neg = gamma_neg
    ...
    def forward(self, logits, targets):
        ...
        targets_unann = (torch.isnan(targets)).float()
        ...
        targets_weights, xs_neg = self.edit_targets_parital_labels(targets, targets_weights, xs_neg)
    ...
    def edit_targets_parital_labels(self, targets, targets_weights, xs_neg, prior_classes=None):
        ...
        targets_weights[torch.isnan(targets)] = 0
...
class PartialSelectiveLoss(nn.Module):
    ...
    def __init__(self, clip=0, gamma_pos=0, gamma_neg=1, gamma_unann=2, alpha_pos=1, alpha_neg=1, alpha_unann=1, prior_path=None, likelihood_topk=5, prior_threshold=0.05):
    ...
    def edit_targets_parital_labels(self, targets, targets_weights, xs_neg, prior_classes=None):
        ...
        if prior_classes is not None:
            if self.prior_threshold:
                ...
                targets_weights += (~torch.isnan(targets)).float()
...
# @torch.jit.script
def negative_backprop_fun_jit(targets, xs_neg_prob, targets_weights, num_top_k: int):
    with torch.no_grad():
